...
votes = []
while True:
    vote = int(input('Opções entre 1 e 23, digite seu voto [0 encerra o programa]: '))
    if vote == 0:
        while True:
            check = input('Tem certeza que deseja encerrar a votação? [S/N]: ').upper().strip()[0]
            if check in 'SN':
                break
        if check == 'S':
            break
